...
def inicializar_banco():
    ...
    com papéis restritos a 'admin' ou 'coordenador', e inicializa um usuário admin padrão com senha criptografada.
    """
    conexao = sqlite3.connect('Cavere.db')
    cursor = conexao.cursor()
    cursor.execute('''
...
def salvar_novo_usuario(username, senha_pura, role='coordenador'):
    """
    Cadastra um novo usuário no banco garantindo que toda senha
    seja SEMPRE criptografada com generate_password_hash da werkzeug.security.
    """
    ...
    try:
        ...
        return False, f"O usuário '{username}' já está cadastrado no sistema."
...
def cadastrar_usuario():
    if request.method == 'POST':
        ...
        sucesso, mensagem = salvar_novo_usuario(username, senha, role)
        if sucesso:
...
def home():
    ...
    cursor.execute('''
        SELECT s.id, co.nome_completo, s.tipo_equipamento, s.quantidade, s.destinatario, 
    ...
    conexao.close()
    return render_template('index.html', equipamentos=equipamentos, coordenadores=coordenadores, solicitacoes=solicitacoes)
...
def gerar():
    ...
    try:
        ...
        # 4. Desenha o documento usando HTML
        html_content = f"""
        <!DOCTYPE html>
        <html lang="pt-BR">
        <head>
        ...
        print(f"📄 PDF gerado e salvo como: {caminho_arquivo}")
...
def salvar_coordenador():
    ...
    try:
        ...
        print(f"Erro ao cadastrar coordenador: {e}")
    finally:
        conexao.close()
...
def devolver(id_equipamento):
    ...
    try:
        ...
        # 2. Carimba a data de devolução na tabela de cautelas (na última saída que estava sem devolução)
        cursor.execute('''
            UPDATE cautelas 
...
def historico():
    conexao = sqlite3.connect('Cavere.db')
    cursor = conexao.cursor()
    # Puxa os dados cruzando as tabelas de Cautelas, Equipamentos e Coordenadores
    cursor.execute('''
        SELECT c.id_cautela, e.tipo, e.modelo, e.patrimonio_sn, co.nome_completo, c.rdo_vinculado, c.data_hora_saida, c.data_hora_devolucao
        FROM cautelas c
        JOIN equipamentos e ON c.id_equipamento = e.id
        JOIN coordenadores co ON c.id_coordenador = co.id
        ORDER BY c.id_cautela DESC
    ''')
    cautelas = cursor.fetchall()
    conexao.close()
    
    return render_template('historico.html', cautelas=cautelas)


# Rota para Acessar/Baixar os PDFs de Cautela
...
def meus_chamados():
    ...
    contadores = {'Pendente': 0, 'Em Operação': 0, 'Devolvido': 0, 'Total': 0}
    ...
    for s in solicitacoes_rows:
        ...
        if st in ('Devolvido', 'Concluído', 'Finalizado'):
            st_label = 'Devolvido'
